...
To tell nox where these environments live, create the file `.config/noxconfig.toml`
with the following:
...
when creating virtualenvs.  The file `.config/noxconfig.toml` should not be tracked by git.
There is also an example file `.config/noxconfig.example.toml` to get you started.
